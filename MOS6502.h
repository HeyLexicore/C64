...
class MOS6502 {
  ...
  private:
    ...
    {AddrMode::Absolute,    "NOP $%04X",     3, 4, nullptr      , 0x0C}, // (undoc) NOP abs
    {AddrMode::Absolute,    "ORA $%04X",     3, 4, &MOS6502::ORA, 0x0D}, // ORA abs
    {AddrMode::Absolute,    "ASL $%04X",     3, 6, &MOS6502::ASL, 0x0E}, // ASL abs
    {AddrMode::Absolute,    "SLO $%04X",     3, 6, nullptr      , 0x0F}, // (undoc) ASL + ORA abs

    {AddrMode::Relative,    "BPL",           2, 2, &MOS6502::BPL, 0x10}, // Branch on Plus
    ...
    {AddrMode::AbsoluteY,   "ORA $%04X,Y",   3, 4, &MOS6502::ORA, 0x19}, // ORA abs,Y
    {AddrMode::Implied,     "NOP",           1, 2, nullptr      , 0x1A}, // (undoc) NOP (1-byte)
    {AddrMode::AbsoluteY,   "SLO $%04X,Y",   3, 7, nullptr      , 0x1B}, // (undoc) ASL + ORA abs,Y
    {AddrMode::AbsoluteX,   "NOP $%04X,X",   3, 4, nullptr      , 0x1C}, // (undoc) NOP abs,X
    {AddrMode::AbsoluteX,   "ORA $%04X,X",   3, 4, &MOS6502::ORA, 0x1D}, // ORA abs,X
    {AddrMode::AbsoluteX,   "ASL $%04X,X",   3, 7, &MOS6502::ASL, 0x1E}, // ASL abs,X
    {AddrMode::AbsoluteX,   "SLO $%04X,X",   3, 7, nullptr      , 0x1F}, // (undoc) ASL + ORA abs,X

    {AddrMode::Absolute,    "JSR $%04X",     3, 6, &MOS6502::JSR, 0x20}, // Jump to Subroutine
    ...
    {AddrMode::Absolute,    "BIT $%04X",     3, 4, &MOS6502::BIT, 0x2C}, // BIT abs
    {AddrMode::Absolute,    "AND $%04X",     3, 4, &MOS6502::AND, 0x2D}, // AND abs
    {AddrMode::Absolute,    "ROL $%04X",     3, 6, &MOS6502::ROL, 0x2E}, // ROL abs
    {AddrMode::Absolute,    "RLA $%04X",     3, 6, nullptr      , 0x2F}, // (undoc) ROL + AND abs

    {AddrMode::Relative,    "BMI",           2, 2, &MOS6502::BMI, 0x30}, // Branch on Minus
    ...
    {AddrMode::AbsoluteY,   "AND $%04X,Y",   3, 4, &MOS6502::AND, 0x39}, // AND abs,Y
    {AddrMode::Implied,     "NOP",           1, 2, nullptr      , 0x3A}, // (undoc) NOP (1-byte)
    {AddrMode::AbsoluteY,   "RLA $%04X,Y",   3, 7, nullptr      , 0x3B}, // (undoc) ROL + AND abs,Y
    {AddrMode::AbsoluteX,   "NOP $%04X,X",   3, 4, nullptr      , 0x3C}, // (undoc) NOP abs,X
    {AddrMode::AbsoluteX,   "AND $%04X,X",   3, 4, &MOS6502::AND, 0x3D}, // AND abs,X
    {AddrMode::AbsoluteX,   "ROL $%04X,X",   3, 7, &MOS6502::ROL, 0x3E}, // ROL abs,X
    {AddrMode::AbsoluteX,   "RLA $%04X,X",   3, 7, nullptr      , 0x3F}, // (undoc) ROL + AND abs,X
    ...
    {AddrMode::Absolute,    "JMP $%04X",     3, 3, &MOS6502::JMP, 0x4C}, // JMP abs
    {AddrMode::Absolute,    "EOR $%04X",     3, 4, &MOS6502::EOR, 0x4D}, // EOR abs
    {AddrMode::Absolute,    "LSR $%04X",     3, 6, &MOS6502::LSR, 0x4E}, // LSR abs
    {AddrMode::Absolute,    "SRE $%04X",     3, 6, nullptr      , 0x4F}, // (undoc) LSR + EOR abs

    {AddrMode::Relative,    "BVC",           2, 2, &MOS6502::BVC, 0x50}, // Branch if Overflow Clear
    ...
    {AddrMode::AbsoluteY,   "EOR $%04X,Y",   3, 4, &MOS6502::EOR, 0x59}, // EOR abs,Y
    {AddrMode::Implied,     "NOP",           1, 2, nullptr      , 0x5A}, // (undoc) NOP (1-byte)
    {AddrMode::AbsoluteY,   "SRE $%04X,Y",   3, 7, nullptr      , 0x5B}, // (undoc) LSR + EOR abs,Y
    {AddrMode::AbsoluteX,   "NOP $%04X,X",   3, 4, nullptr      , 0x5C}, // (undoc) NOP abs,X
    {AddrMode::AbsoluteX,   "EOR $%04X,X",   3, 4, &MOS6502::EOR, 0x5D}, // EOR abs,X
    {AddrMode::AbsoluteX,   "LSR $%04X,X",   3, 7, &MOS6502::LSR, 0x5E}, // LSR abs,X
    {AddrMode::AbsoluteX,   "SRE $%04X,X",   3, 7, nullptr      , 0x5F}, // (undoc) LSR + EOR abs,X
    ...
    {AddrMode::Indirect,    "JMP ($%02X)",   3, 5, &MOS6502::JMP, 0x6C}, // JMP (addr)
    {AddrMode::Absolute,    "ADC $%04X",     3, 4, &MOS6502::ADC, 0x6D}, // ADC abs
    {AddrMode::Absolute,    "ROR $%04X",     3, 6, &MOS6502::ROR, 0x6E}, // ROR abs
    {AddrMode::Absolute,    "RRA $%04X",     3, 6, nullptr      , 0x6F}, // (undoc) ROR + ADC abs

    {AddrMode::Relative,    "BVS",           2, 2, &MOS6502::BVS, 0x70}, // Branch if Overflow Set
    ...
    {AddrMode::AbsoluteY,   "ADC $%04X,Y",   3, 4, &MOS6502::ADC, 0x79}, // ADC abs,Y
    {AddrMode::Implied,     "NOP",           1, 2, nullptr      , 0x7A}, // (undoc) NOP (1-byte)
    {AddrMode::AbsoluteY,   "RRA $%04X,Y",   3, 7, nullptr      , 0x7B}, // (undoc) ROR + ADC abs,Y
    {AddrMode::AbsoluteX,   "NOP $%04X,X",   3, 4, nullptr      , 0x7C}, // (undoc) NOP abs,X
    {AddrMode::AbsoluteX,   "ADC $%04X,X",   3, 4, &MOS6502::ADC, 0x7D}, // ADC abs,X
    {AddrMode::AbsoluteX,   "ROR $%04X,X",   3, 7, &MOS6502::ROR, 0x7E}, // ROR abs,X
    {AddrMode::AbsoluteX,   "RRA $%04X,X",   3, 7, nullptr      , 0x7F}, // (undoc) ROR + ADC abs,X
    ...
    {AddrMode::Absolute,    "STY $%04X",     3, 4, &MOS6502::STY, 0x8C}, // STY abs
    {AddrMode::Absolute,    "STA $%04X",     3, 4, &MOS6502::STA, 0x8D}, // STA abs
    {AddrMode::Absolute,    "STX $%04X",     3, 4, &MOS6502::STX, 0x8E}, // STX abs
    {AddrMode::Absolute,    "SAX $%04X",     3, 4, nullptr      , 0x8F}, // (undoc) Store A & X at abs

    {AddrMode::Relative,    "BCC",           2, 2, &MOS6502::BCC, 0x90}, // Branch if Carry Clear
    ...
    {AddrMode::AbsoluteY,   "STA $%04X,Y",   3, 5, &MOS6502::STA, 0x99}, // STA abs,Y
    {AddrMode::Implied,     "TXS",           1, 2, &MOS6502::TXS, 0x9A}, // Transfer X to Stack Ptr
    {AddrMode::AbsoluteY,   "TAS $%04X,Y",   3, 5, nullptr      , 0x9B}, // (undoc) Transfer A & X to SP, store
    {AddrMode::AbsoluteX,   "SHY $%04X,X",   3, 5, nullptr      , 0x9C}, // (undoc) Store Y & (high byte+1)
    {AddrMode::AbsoluteX,   "STA $%04X,X",   3, 5, &MOS6502::STA, 0x9D}, // STA abs,X
    {AddrMode::AbsoluteY,   "SHX $%04X,Y",   3, 5, nullptr      , 0x9E}, // (undoc) Store X & (high byte+1)
    {AddrMode::AbsoluteY,   "AHX $%04X,Y",   3, 5, nullptr      , 0x9F}, // (undoc) Store A & X & (high byte+1)
    ...
    {AddrMode::Absolute,    "LDY $%04X",     3, 4, &MOS6502::LDY, 0xAC}, // LDY abs
    {AddrMode::Absolute,    "LDA $%04X",     3, 4, &MOS6502::LDA, 0xAD}, // LDA abs
    {AddrMode::Absolute,    "LDX $%04X",     3, 4, &MOS6502::LDX, 0xAE}, // LDX abs
    {AddrMode::Absolute,    "LAX $%04X",     3, 4, nullptr      , 0xAF}, // (undoc) Load A & X from abs

    {AddrMode::Relative,    "BCS",           2, 2, &MOS6502::BCS, 0xB0}, // Branch if Carry Set
    ...
    {AddrMode::AbsoluteY,   "LDA $%04X,Y",   3, 4, &MOS6502::LDA, 0xB9}, // LDA abs,Y
    {AddrMode::Implied,     "TSX",           1, 2, &MOS6502::TSX, 0xBA}, // Transfer SP to X
    {AddrMode::AbsoluteY,   "LAS $%04X,Y",   3, 4, nullptr      , 0xBB}, // (undoc) Load A,X,SP = SP & M
    {AddrMode::AbsoluteX,   "LDY $%04X,X",   3, 4, &MOS6502::LDY, 0xBC}, // LDY abs,X
    {AddrMode::AbsoluteX,   "LDA $%04X,X",   3, 4, &MOS6502::LDA, 0xBD}, // LDA abs,X
    {AddrMode::AbsoluteY,   "LDX $%04X,Y",   3, 4, &MOS6502::LDX, 0xBE}, // LDX abs,Y
    {AddrMode::AbsoluteY,   "LAX $%04X,Y",   3, 4, nullptr      , 0xBF}, // (undoc) Load A & X from abs,Y
    ...
    {AddrMode::Absolute,    "CPY $%04X",     3, 4, &MOS6502::CPY, 0xCC}, // CPY abs
    {AddrMode::Absolute,    "CMP $%04X",     3, 4, &MOS6502::CMP, 0xCD}, // CMP abs
    {AddrMode::Absolute,    "DEC $%04X",     3, 6, &MOS6502::DEC, 0xCE}, // DEC abs
    {AddrMode::Absolute,    "DCP $%04X",     3, 6, nullptr      , 0xCF}, // (undoc) DEC + CMP abs

    {AddrMode::Relative,    "BNE",           2, 2, &MOS6502::BNE, 0xD0}, // Branch if Not Equal
    ...
    {AddrMode::AbsoluteY,   "CMP $%04X,Y",   3, 4, &MOS6502::CMP, 0xD9}, // CMP abs,Y
    {AddrMode::Implied,     "NOP",           1, 2, nullptr      , 0xDA}, // (undoc) NOP (1-byte)
    {AddrMode::AbsoluteY,   "DCP $%04X,Y",   3, 7, nullptr      , 0xDB}, // (undoc) DEC + CMP abs,Y
    {AddrMode::AbsoluteX,   "NOP $%04X,X",   3, 4, nullptr      , 0xDC}, // (undoc) NOP abs,X
    {AddrMode::AbsoluteX,   "CMP $%04X,X",   3, 4, &MOS6502::CMP, 0xDD}, // CMP abs,X
    {AddrMode::AbsoluteX,   "DEC $%04X,X",   3, 7, &MOS6502::DEC, 0xDE}, // DEC abs,X
    {AddrMode::AbsoluteX,   "DCP $%04X,X",   3, 7, nullptr      , 0xDF}, // (undoc) DEC + CMP abs,X
    ...
    {AddrMode::Absolute,    "CPX $%04X",     3, 4, &MOS6502::CPX, 0xEC}, // CPX abs
    {AddrMode::Absolute,    "SBC $%04X",     3, 4, &MOS6502::SBC, 0xED}, // SBC abs
    {AddrMode::Absolute,    "INC $%04X",     3, 6, &MOS6502::INC, 0xEE}, // INC abs
    {AddrMode::Absolute,    "ISC $%04X",     3, 6, nullptr      , 0xEF}, // (undoc) INC + SBC abs

    {AddrMode::Relative,    "BEQ",           2, 2, &MOS6502::BEQ, 0xF0}, // Branch if Equal
    ...
    {AddrMode::AbsoluteY,   "SBC $%04X,Y",   3, 4, &MOS6502::SBC, 0xF9}, // SBC abs,Y
    {AddrMode::Implied,     "NOP",           1, 2, nullptr      , 0xFA}, // (undoc) NOP (1-byte)
    {AddrMode::AbsoluteY,   "ISC $%04X,Y",   3, 7, nullptr      , 0xFB}, // (undoc) INC + SBC abs,Y
    {AddrMode::AbsoluteX,   "NOP $%04X,X",   3, 4, nullptr      , 0xFC}, // (undoc) NOP abs,X
    {AddrMode::AbsoluteX,   "SBC $%04X,X",   3, 4, &MOS6502::SBC, 0xFD}, // SBC abs,X
    {AddrMode::AbsoluteX,   "INC $%04X,X",   3, 7, &MOS6502::INC, 0xFE}, // INC abs,X
    {AddrMode::AbsoluteX,   "ISC $%04X,X",   3, 7, nullptr      , 0xFF}  // (undoc) INC + SBC abs,X
    };
    void ADC(AddrMode);
    void AND(AddrMode);
    void ASL(AddrMode);
    ...
    uint16_t getAddress(OPERAND &op);
  public:
    OPERAND fetch();
    int cycle();
